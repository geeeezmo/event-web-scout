...
def test_invalid_config_schema_file():
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        init(config_schema_file_name='../tests/config/invalid_config_schema.json')
    assert pytest_wrapped_e.type == SystemExit
...
def test_invalid_config_file():
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        init(config_file_name='../tests/config/invalid_config.json')
    assert pytest_wrapped_e.type == SystemExit
    assert pytest_wrapped_e.value.code == 1010
...
def test_config_file_schema_mismatch():
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        init(config_file_name='../tests/config/config_with_schema_mismatch.json')
    assert pytest_wrapped_e.type == SystemExit
